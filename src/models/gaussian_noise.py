...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, x_start: torch.Tensor, descriptors: torch.Tensor, epoch, voltage: torch.Tensor, raw_ppm: torch.Tensor, raw_molwt: torch.Tensor) -> Tensor:
        """
        ...
        phys_loss = self.electro_loss(predicted_signal, x_start, voltage)
        loss_l1 = F.l1_loss(predicted_signal, x_start)
        zeros = torch.tensor(0.0, device=device)
    
        return loss_l1, phys_loss * 0.006, zeros, zeros
    @torch.no_grad()
